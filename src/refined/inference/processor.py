...
from refined.data_types.base_types import Entity, Span, Span_UMLS, Entity_UMLS
from refined.data_types.doc_types import Doc, Doc_UMLS
from refined.data_types.modelling_types import BatchedElementsTns, ModelReturn, BatchedElementsTns_UMLS, ModelReturn_UMLS
from refined.doc_preprocessing.preprocessor import PreprocessorInferenceOnly, Preprocessor, PreprocessorInferenceOnly_UMLS
from refined.model_components.config import ModelConfig
from refined.model_components.refined_model import RefinedModel, RefinedModel_UMLS
...
from refined.utilities.preprocessing_utils import convert_doc_to_tensors, convert_batch_elements_to_batched_tns, convert_doc_to_tensors_UMLS, convert_batch_elements_to_batched_tns_UMLS
...
class Refined_UMLS(object):
    ...
    def __init__(
            self,
            model_file_or_model: Union[str, RefinedModel_UMLS],
            model_config_file_or_model_config: Union[str, ModelConfig],
            entity_set: Optional[str] = None,
            data_dir: Optional[str] = None,
            n_gpu: int = 1,
        ...
        if preprocessor is None:
            assert entity_set is not None, "`entity_set` must be provided when preprocessor is not provided as " \
                                           "an argument"
            assert data_dir is not None, "`data_dir` must be provided when preprocessor is not provided as an argument"
            self.preprocessor = PreprocessorInferenceOnly_UMLS.from_model_config_file(
                filename=model_config_file_or_model_config,
                data_dir=data_dir,
                debug=debug,
                entity_set="umls",
        ...
        if isinstance(model_file_or_model, RefinedModel_UMLS) or isinstance(model_file_or_model, DataParallel):
            # RefinedModel or DataParallel model
            self.model = model_file_or_model
        else:
            assert isinstance(model_config_file_or_model_config, str), f"model_config_file_or_model_config must be" \
                                                                       f"a string when model_file_or_model is a string."
            self.model = RefinedModel_UMLS.from_pretrained(
                model_file=model_file_or_model,
                model_config_file=model_config_file_or_model_config,
                preprocessor=self.preprocessor,
                use_precomputed_descriptions=use_precomputed_descriptions
    ...
    def process_text(
            self,
            text: str,
            spans: Optional[List[Span_UMLS]] = None,
            ner_threshold: float = 0.5,
            max_batch_size: int = 16,
    ) -> List[Span_UMLS]:
        """
        ...
        all_spans = []
        if spans is not None:
            doc = Doc_UMLS.from_text_with_spans(
                text, spans, self.preprocessor
            )
        else:
            doc = Doc_UMLS.from_text(
                text,
                preprocessor=self.preprocessor
            )
        tns: Iterable[BatchedElementsTns_UMLS] = convert_doc_to_tensors_UMLS(
            doc,
            self.preprocessor,
            collate=True,
            max_batch_size=max_batch_size,
    ...
    def process_text_batch(
            self,
            texts: List[str],
            spanss: Optional[List[List[Span_UMLS]]] = None,
            ner_threshold: float = 0.5,
            max_batch_size: int = 16,
            apply_class_check: bool = False,
            sort_by_tokens: bool = True
        ...
        if spanss is not None:
            ...
            for i, text in enumerate(texts):
                doc = Doc_UMLS.from_text(
                    text, preprocessor=self.preprocessor, doc_id=str(i)
                )
                docs.append(doc)
        batch_elements = [elem for doc in docs for elem in doc.to_batch_elements(
            preprocessor=self.preprocessor, override_max_seq=self.max_seq
        )]
        tns: Iterable[BatchedElementsTns_UMLS] = convert_batch_elements_to_batched_tns_UMLS(
                batch_elements,
                self.preprocessor,
                max_batch_size=max_batch_size,
                sort_by_tokens=sort_by_tokens,
    ...
    def from_pretrained(
        ...
        model_files = {
            ...
            'model_description_embeddings': {
                # TODO: remove hard-coded size of entity sets
                'umls': os.path.join(model_name, 'precomputed_entity_descriptions_emb_umls_88152-300.np')
            }[entity_set],
        }
        # model_description_embeddings
        if not os.path.exists(model_files["model_description_embeddings"]):
