...
from refined.sapbert.metric_learning import *
...
class Model_Wrapper(object):
    ...
    def retrieve_candidate_cuda(self, score_matrix, topk, batch_size=128, show_progress=False):
        ...
        for i in tqdm(np.arange(0, score_matrix.shape[0], batch_size), disable=not show_progress):
            ...
            matrix_sorted_scores, _ = torch.sort(score_matrix_tmp, dim=1, descending=True)
            matrix_sorted_scores = matrix_sorted_scores[:, :topk].cpu()
            if res is None:
                res = matrix_sorted
                res_scores = matrix_sorted_scores
            else:
                res = torch.cat([res, matrix_sorted], axis=0)
                res_scores = torch.cat([res_scores, matrix_sorted_scores], axis=0)

        return res.numpy(), res_scores.numpy()
        ...
        with torch.no_grad():
            ...
            for start in iterations:
                ...
                if agg_mode == "cls":
                    ...
                    batch_dense_embeds = (last_hidden_state * batch_tokenized_names_cuda['attention_mask'].unsqueeze(
                        -1)).sum(1) / batch_tokenized_names_cuda['attention_mask'].sum(-1).unsqueeze(-1)
                else:
