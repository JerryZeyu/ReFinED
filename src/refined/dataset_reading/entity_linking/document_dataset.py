...
from refined.doc_preprocessing.preprocessor import PreprocessorInferenceOnly, PreprocessorInferenceOnly_UMLS
from refined.utilities.dataset_utils import mask_mentions, mask_mentions_UMLS
from refined.utilities.preprocessing_utils import collate_batch_elements_tns, convert_batch_element_to_tensors, collate_batch_elements_tns_UMLS, convert_batch_element_to_tensors_UMLS
...
class DocDataset_UMLS(Dataset):
    ...
    def __init__(
        ...
        self.max_seq = max_seq
        self.mask = mask
        self.batch_elements: List[BatchElement_UMLS] = []
        self.load_docs(docs=list(docs))

    def load_docs(self, docs: List[Doc_UMLS]):
        for doc in docs:
            self.batch_elements.extend(
                [
                    b
    ...
    def __getitem__(self, index: int) -> BatchElementTns_UMLS:
        """
        Returns the tensors for a single batch element.
        :param index: the index of the batch element to return
        :return: batch element converted to a tuple of tensors
        ...
        if self.mask:
            ...
            batch_elem = self.batch_elements[index]
        return convert_batch_element_to_tensors_UMLS(
            batch_element=batch_elem, processor=self.preprocessor
        )
    ...
    def collate(self, batch_elements_tns: List[BatchElementTns_UMLS]) -> BatchedElementsTns_UMLS:
        """
        Collates a list of tensors for batch elements into a single batch.
        - Pads tensors to longest length in batch
        - Corrects masks to prevent information loss
        ...
        :return: a tuple of tensors where all tensors in the list are vertically stacked
        """
        return collate_batch_elements_tns_UMLS(
            batch_elements_tns=batch_elements_tns, token_pad_value=self.preprocessor.pad_id
        )
